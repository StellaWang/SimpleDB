...
public class JoinOptimizer {
    ...
    public double estimateJoinCost(LogicalJoinNode j, int card1, int card2,
        ...
        if (j instanceof LogicalSubplanJoinNode) {
            ...
            // join.
        	// Since a basic nested-loops join is used, use the function
        	// joincost(t1 join t2) = scancost(t1) + ntups(t1)*scancost(t2) //I/O cost
    ...
    public static int estimateTableJoinCardinality(Predicate.Op joinOp,
        ...
        if (joinOp == Predicate.Op.EQUALS)
        {
			...
			//If both are primary keys, card1 and card2 are same, return either one is fine
        	//if neither is primary key, return the size of the larger of the two tables
        		if (card1 >= card2)
        			card = card1;
        		else
					card = card2;
			...
			//If it's not equality joins,
        	//assume 0.3 of the cross-product is emitted by range scans
        	card = (int) (0.7*card1*card2);
		}
 ...
 /*   	PlanCache optjoin = new PlanCache();
    	...
    	for (int i = 1; i <= joins.size(); i++) {
    		Set<Set<LogicalJoinNode>> subsetsOfLenI = enumerateSubsets(joins, i);
    		
    		for (Set<LogicalJoinNode> s : subsetsOfLenI) {
    			CostCard bestCCSoFar = new CostCard();
    			bestCCSoFar.cost = Double.MAX_VALUE;
    			
    			for (LogicalJoinNode joinToRemove : s) {
    				CostCard cc = computeCostAndCardOfSubplan(
    						stats, filterSelectivities, joinToRemove, s, bestCCSoFar.cost, optjoin);
    				if (cc != null) {
    					bestCCSoFar = cc;
    				}
    			}
    			optjoin.addPlan(s, bestCCSoFar.cost, bestCCSoFar.card, bestCCSoFar.plan);
    		}
    	}
